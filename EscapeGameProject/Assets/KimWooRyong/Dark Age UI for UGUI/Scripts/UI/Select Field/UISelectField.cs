...
namespace DuloGames.UI
{
	...
	public class UISelectField : Toggle {
		...
		{
			if (this.animator == null || !this.animator.enabled || !this.animator.isActiveAndEnabled || !this.animator.hasBoundPlayables || this.animator.runtimeAnimatorController == null || string.IsNullOrEmpty(trigger))
				return;
		...
		private void TriggerListAnimation(string trigger)
		{
			...
			if (animator == null || !animator.isActiveAndEnabled || !animator.hasBoundPlayables || animator.runtimeAnimatorController == null)
				return;
